...
SELECT  DISTINCT(WINT.CODSEC), WINT.SECAO, 
        (SELECT DISTINCT(U.CODSUPERVISOR) FROM PONTUAL.PCUSUARI U WHERE U.CODSUPERVISOR = WINT.CODSUPERVISOR) AS CODSUPERVISOR,
        ROUND(SUM(WINT.VLVENDA),2) AS "TOTAL VLVENDA", 
        ROUND(
                CASE
...
WHERE WINT.CODSUPERVISOR = {sup}
GROUP BY WINT.CODSEC, WINT.SECAO, DU1.QTDIASVENDAS, DU2.QTDIASVENDAS, DU3.QTDIASVENDAS, WINT.CODSUPERVISOR

SELECT WINT.CODPROD || '', WINT.QTVENDA || ''
FROM (SELECT PCPRODUT.CODPROD,
       ...
       (  (  (  SUM (NVL (vendas.vltabela, 0) - NVL (devolucao.vltabeladevolucao, 0))
                     - SUM (NVL (vendas.vlvenda, 0) - NVL (devolucao.vldevolucao, 0))
                    )
                  / DECODE (SUM (  NVL (vendas.vltabela, 0)
                                 - NVL (devolucao.vltabeladevolucao, 0)
                                ),
                            0, 1,
                            SUM (  NVL (vendas.vltabela, 0)
                                 - NVL (devolucao.vltabeladevolucao, 0)
                                )
                           )
                 )
       ...
       SUM(NVL(DEVOLUCAO.VLDEVOLUCAOBNF, 0)) VLDEVOLUCAOBNF,
 ...
 ( ( SUM (NVL (DEVOLUCAO.VLDEVOLUCAO, 0)) + SUM (NVL (DEVOLUCAO.VLDEVOLUCAOBNF, 0))) / ( CASE WHEN (SUM (NVL (VENDAS.VLVENDA, 0)) = 0) THEN 1 ELSE SUM (NVL (VENDAS.VLVENDA, 0)) END ) ) * 100 VLTROCA, 
...
FROM  (SELECT CODPROD,
              ...
              SUM(NVL(VLTABELA,0)) VLTABELA, SUM(NVL(VLREPASSEBNF, 0)) VLREPASSEBNF FROM (  SELECT PCMOV.CODCLI, 
         PCATIVI.RAMO, 
         PCATIVI.CODATIV, 
...
WHERE P.CODFORNEC = PCFORNEC.CODFORNEC AND NVL(P.REVENDA,'S')  = 'S' ) QTMIXCADNOVO,
 ...
 WHERE PCMOV.NUMTRANSVENDA = PCNFSAID.NUMTRANSVENDA
   ...
   AND PCMOV.DTMOV BETWEEN TO_DATE('{dtIni}', 'DD-MM-YYYY') AND TO_DATE('{dtFim}', 'DD-MM-YYYY') 
  ...
  AND PCMOV.CODOPER <> 'SO' 
   ...
   AND PCNFSAID.DTSAIDA BETWEEN TO_DATE('{dtIni}', 'DD-MM-YYYY') AND TO_DATE('{dtFim}', 'DD-MM-YYYY')
      ...
      ( SELECT CODPROD,
               ...
               SUM (VLDEVOLUCAOBNF) VLDEVOLUCAOBNF,
               SUM(TOTPESO) TOTPESO,
               SUM(VOLUME) VOLUME,
               SUM(DEVOLTAB) VLTABELADEVOLUCAO,
               SUM(NVL(VLBONIFIC,0)) VLBONIFIC, 
 ...
 ,(SELECT DISTINCT CASE                                          
   ...
   AND PED.DATA BETWEEN TO_DATE('{dtIni}', 'DD-MM-YYYY') AND TO_DATE('{dtFim}', 'DD-MM-YYYY')
      ) TEMVENDATV8 
...
AND NVL(PCNFSAID.CODSUPERVISOR,PCUSUARI.CODSUPERVISOR) = PCSUPERV.CODSUPERVISOR
 ...
 AND DECODE(PCNFSAID.CODGERENTE,NULL,PCSUPERV.CODGERENTE,PCNFSAID.CODGERENTE) = PCGERENTE.CODGERENTE
   ...
   AND PCNFENT.DTENT BETWEEN TO_DATE('{dtIni}', 'DD-MM-YYYY') AND TO_DATE('{dtFim}', 'DD-MM-YYYY')
   ...
   AND PCMOV.DTMOV BETWEEN TO_DATE('{dtIni}', 'DD-MM-YYYY') AND TO_DATE('{dtFim}', 'DD-MM-YYYY')
        ...
        GROUP BY CODPROD) DEVOLUCAO,
     PONTUAL.PCPRODUT, PONTUAL.PCMARCA
WHERE PCPRODUT.CODPROD = VENDAS.CODPROD(+)
  AND PCPRODUT.CODMARCA = PCMARCA.CODMARCA(+)
  AND PCPRODUT.codfornec not in (1992)
  AND PCPRODUT.CODPROD = DEVOLUCAO.CODPROD(+)
  AND ((NVL(QTVENDA, 0) <> 0) OR (NVL(VLVENDA, 0) <> 0) OR
       (NVL(DEVOLUCAO.VLDEVOLUCAO, 0) <> 0) OR (NVL(DEVOLUCAO.QTDEVOLUCAO, 0) <> 0) OR

WITH VL AS (
SELECT CODUSUR, NOME, VLVENDA, VLBONIFIC
FROM
...
WHERE P.CODFORNEC = PCFORNEC.CODFORNEC AND NVL(P.REVENDA,'S')  = 'S' ) QTMIXCADNOVO,
 ...
 WHERE PCMOV.NUMTRANSVENDA = PCNFSAID.NUMTRANSVENDA
   ...
   AND PCMOV.DTMOV BETWEEN  TRUNC(SYSDATE, 'MM') AND LAST_DAY(SYSDATE)
  ...
  AND PCMOV.CODOPER <> 'SO' 
   ...
   AND ( PCNFSAID.CODUSUR  IN ( {rca} )) 
   AND PCNFSAID.DTSAIDA BETWEEN  TRUNC(SYSDATE, 'MM') AND LAST_DAY(SYSDATE)
...
AND NVL(PCNFSAID.CODSUPERVISOR,PCUSUARI.CODSUPERVISOR) = PCSUPERV.CODSUPERVISOR
 ...
 AND DECODE(PCNFSAID.CODGERENTE,NULL,PCSUPERV.CODGERENTE,PCNFSAID.CODGERENTE) = PCGERENTE.CODGERENTE
   ...
   AND PCNFENT.DTENT BETWEEN  TRUNC(SYSDATE, 'MM') AND LAST_DAY(SYSDATE)
...
SELECT 
        5 AS "ORDER",
        '% (TROCA - FLEX)' AS "§",
        ((A.VLBONIFIC + B.GERADO) / A.VLVENDA) * 100 AS "¬"
FROM    
        VL A,
        FLEX B
WHERE   

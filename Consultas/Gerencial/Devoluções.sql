...
SELECT
    ...
    'R$' || TRUNC((CASE WHEN A.CODDEVOL = 106 THEN 0 ELSE
    (CASE WHEN A.VLTOTAL = 0 AND A.VLBONIFIC = 0 THEN A.VLTOTGER 
          WHEN A.VLTOTAL = 0 AND A.VLBONIFIC > 0 THEN A.VLBONIFIC 
          WHEN A.VLTOTAL > 0 THEN A.VLTOTAL ELSE A.VLTOTGER END) END), 2) VALOR,
    'R$' || TRUNC((CASE WHEN A.CODDEVOL IN (11, 5, 3, 29, 6, 17, 1, 28, 26, 30, 27, 31, 33, 14, 46, 13, 9, 32, 21, 110, 111)
               THEN TRUNC((A.VLTOTGER * 0.10),2) ELSE 0 END),2) "ABAT. RCA",
    (CASE WHEN A.VLBONIFIC > 0 THEN 'SIM' ELSE 'NÃO' END) "BNF",    
    A.CODDEVOL || ' - ' || B.MOTIVO AS MOTIVO, 
    (CASE WHEN A.CODDEVOL IN (7, 4, 41, 45, 34, 15, 25, 12, 18, 105, 36, 43, 103, 16, 104, 8, 20, 19, 10, 42) THEN 'L'
          WHEN A.CODDEVOL IN (11, 5, 3, 29, 6, 17, 1, 28, 26, 30, 27, 31, 33, 14, 46, 13, 9, 32, 21, 110, 111) THEN 'C'
          WHEN A.CODDEVOL IN (38, 39, 22, 35, 109) THEN 'A'
...
FROM
    PCNFENT A
JOIN
    PCTABDEV B ON A.CODDEVOL = B.CODDEVOL
JOIN 
    PCCLIENT C ON C.CODCLI = A.CODFORNEC
JOIN
    PCUSUARI USUR ON USUR.CODUSUR = C.CODUSUR1   
JOIN
    PCEMPR E ON A.codmotoristadevol = E.MATRICULA
JOIN    
    RELWINT W ON A.NUMNOTA = W.NUMNOTA
WHERE
    A.CODDEVOL IS NOT NULL
AND
    A.DTENT BETWEEN '{dtIni}' AND '{dtFim}'
AND 
    W.CODSUPERVISOR IN ({sup})
AND 
    A.CODFILIAL = 3    
AND
...
ORDER BY
    MOTIVO, CLIENTE, "OBSERVAÇÕES" DESC
 SELECT NVL(VENDAS.CODUSUR, DEVOLUCAO.CODUSURDEVOL) CODUSUR, 
        ...
        (CASE WHEN VENDAS.CODUSUR = 160 THEN 500 -- SOLICITADO DIA 21/06
              WHEN VENDAS.CODUSUR = 2 THEN  2200 -- SOLICITADO NO DIA 07-06-24 | 400 SOLICITADO DIA 21/06 | 600 solicitado 28/06
              WHEN VENDAS.CODUSUR = 156 THEN  180 -- SOLICITADO DIA 21/06
              WHEN VENDAS.CODUSUR = 172 THEN  500 -- Solicitado dia 28/06
...
WHERE P.CODFORNEC = PCFORNEC.CODFORNEC AND NVL(P.REVENDA,'S')  = 'S' ) QTMIXCADNOVO,
 ...
 WHERE PCMOV.NUMTRANSVENDA = PCNFSAID.NUMTRANSVENDA
   ...
   AND PCMOV.DTMOV BETWEEN LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -1)) + 1 AND LAST_DAY(TRUNC(SYSDATE, 'MM'))
  ...
  AND PCMOV.CODOPER <> 'SO' 
   ...
   AND PCNFSAID.DTSAIDA BETWEEN LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -1)) + 1 AND LAST_DAY(TRUNC(SYSDATE, 'MM'))
...
PCMETA.CODIGO,
 ...
 WHERE PCMETA.CODUSUR = PCUSUARI.CODUSUR
   ...
   AND PCMETA.DATA BETWEEN LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -1)) + 1 AND LAST_DAY(TRUNC(SYSDATE, 'MM'))
 ...
 ,(SELECT DISTINCT CASE                                          
   ...
   AND PED.DATA BETWEEN LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -1)) + 1 AND LAST_DAY(TRUNC(SYSDATE, 'MM'))
      ) TEMVENDATV8 
...
AND NVL(PCNFSAID.CODSUPERVISOR,PCUSUARI.CODSUPERVISOR) = PCSUPERV.CODSUPERVISOR
 ...
 AND DECODE(PCNFSAID.CODGERENTE,NULL,PCSUPERV.CODGERENTE,PCNFSAID.CODGERENTE) = PCGERENTE.CODGERENTE
   ...
   AND PCNFENT.DTENT BETWEEN LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -1)) + 1 AND LAST_DAY(TRUNC(SYSDATE, 'MM'))

/*
    CONSULTA PARA GERAR INSERTS DE METAS DE FORMA AUTOMATICA
    BY: SAMMUEL G MARTINS
    FOR: PREMIUM DISTRIBUIDORA
*/
WITH 
DIAS_UTEIS_1 AS (
...
SELECT  REPLACE('INSERT INTO PCMETA (CODIGO, CODFILIAL, CODUSUR, TIPOMETA, DATA, CLIPOSPREV, ROTINALANC, CODIGO2, PERCRATEIO) VALUES (' ||
        WINT.CODSEC ||', '|| '3' ||', '|| WINT.CODUSUR ||', '|| '"S"' ||', '|| '"' || (SELECT TO_CHAR(TRUNC(SYSDATE, 'MM'), 'DD-MON-YYYY') FROM DUAL) || '"' ||', '||
        ROUND(CASE WHEN (((SUM(WINT.VLVENDA) / (DU1.QTDIASVENDAS + DU2.QTDIASVENDAS)) * DU3.QTDIASVENDAS) + (((SUM(WINT.VLVENDA) / (DU1.QTDIASVENDAS + DU2.QTDIASVENDAS)) * DU3.QTDIASVENDAS)*0.05)) < 1000 THEN 1000
                ELSE (((SUM(WINT.VLVENDA) / (DU1.QTDIASVENDAS + DU2.QTDIASVENDAS)) * DU3.QTDIASVENDAS) + (((SUM(WINT.VLVENDA) / (DU1.QTDIASVENDAS + DU2.QTDIASVENDAS)) * DU3.QTDIASVENDAS)*0.05)) END, 0) ||', '||
        '0, 0, 0); --SECAO: ' || WINT.SECAO || ' | RCA: ' || WINT.CODUSUR, '"', '''') AS "--INSERT'S META RCA SECAO"
...
WHERE WINT.CODUSUR in (SELECT CODUSUR FROM PONTUAL.PCUSUARI WHERE NOME LIKE 'PMU%' AND CODUSUR NOT IN (2,160,10))
AND WINT.CODSEC NOT IN (120425, 120423, 10043, 6004, 1033) --SECOES INATIVAS
GROUP BY WINT.CODSEC, WINT.SECAO, DU1.QTDIASVENDAS, DU2.QTDIASVENDAS, DU3.QTDIASVENDAS, WINT.CODUSUR
ORDER BY WINT.CODSEC DESC 

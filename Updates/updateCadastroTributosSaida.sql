DECLARE
  CODPROD_PRODUTO_MODELO NUMBER := 18665;  -- INSERIR O CÓDIGO DO PRODUTO QUE FOI FEITO O PREENCHIMENTO MANUAL
  CODFORNEC_1 NUMBER := 1841;  -- INSERIR O CÓDIGO DO FORNECEDOR
BEGIN
---------------------------------------------------------------------------------------------------------------  
UPDATE PCTABPR A
  SET 
...
AND CODPROD NOT IN (18663,18771,18772,18773,18774,18787));
END;
---------------------------------------------------------------------------------------------------------------  
SELECT DISTINCT PCTABPR.CODPROD                
 ...
 WHERE ((PCTABPR.NUMREGIAO = PCREGIAO.NUMREGIAO  
   ...
   AND CODPROD IN (SELECT CODPROD FROM PCPRODUT WHERE CODFORNEC = 1841 AND CODPROD NOT IN (18663,18771,18772,18773,18774,18787) AND CODPROD IN (18665))
